...
class RegistroForm(forms.ModelForm):
    class Meta:
        ...
        def __init__(self, *args, **kwargs) :
            ...
            self.fields['formacao_florestal'] = ModelChoiceField(queryset=FormacaoFlorestal.objects.all().order_by('nome'),
                                                                 to_field_name='nome',
                                                                 empty_label="Selecione a Formação Florestal")
            self.fields['especie'] = ModelChoiceField(queryset=Especie.objects.all().order_by('nome'),
                                                      to_field_name='nome',
